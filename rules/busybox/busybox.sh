...
configure()
{
    make O=$COMPONENT_BUILD -C $COMPONENT_SRC defconfig
    ...
    yes "" | make O=$COMPONENT_BUILD -C $COMPONENT_SRC oldconfig
...
compile()
{
    make V=1 O=$COMPONENT_BUILD -C $COMPONENT_SRC -j$CONFIG_NUMCPUS
}

mkpackage()
{
    make O=$COMPONENT_BUILD -C $COMPONENT_SRC install
}
